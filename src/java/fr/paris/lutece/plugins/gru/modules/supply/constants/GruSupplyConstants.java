package fr.paris.lutece.plugins.gru.modules.supply.constants;

public class GruSupplyConstants {

	// REST CONSTANTS
    public static final String PLUGIN_NAME = "grusupply/";
    public static final String MODULE_NAME_ES = "elasticsearch/";
    public static final String MODULE_NAME_DB = "database/";
    
    // REST PATH
    
    public static final String PATH_ELASTIC_AUTOCOMPLETION = "autocompletion";
    
    
    // EXIT STATUS

    public static final String STATUS_201 = "{" + "\"status\":" + "\"201\"" + "}";
    public static final String STATUS_404 = "{" + "\"status\":" + "\"404\"" + "}";
}
